...
function buildGrowth(measurements, baby) {
  if (!measurements) return null;
  const wUnit = baby?.weight_unit ?? 'lb_oz';
...
function BabyDashboard() {
  ...
  const growth = buildGrowth(measurements, selectedBaby);
    ...
    <div>
        ...
        <div className="chart-card">
          ...
          <ResponsiveContainer width="100%" height={230}>
            <LineChart data={growth} margin={{ top: 4, right: 8, bottom: 0, left: -12 }}>
              ...
              <YAxis yAxisId="w" tick={{ fontSize: 11 }} domain={['auto', 'auto']} />
              <YAxis yAxisId="l" orientation="right" tick={{ fontSize: 11 }} domain={['auto', 'auto']} />
              ...
              <Line
                ...
                type="monotone"
                dataKey="weight"
                name={`Weight (${weightUnitLabel(wUnit)})`}
      ...
      {!hasData ? (
        ...
        <>
          ...
          <div className="chart-card">
            <p className="chart-title">Volume per day (ml)</p>
            <ResponsiveContainer width="100%" height={210}>
              <LineChart data={daily} margin={{ top: 4, right: 16, bottom: 0, left: -16 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#eee" vertical={false} />
                <XAxis dataKey="label" tick={{ fontSize: 11, fontWeight: 700 }} interval="preserveStartEnd" />
                <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
                <Tooltip contentStyle={tooltipStyle} />
